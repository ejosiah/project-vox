...
def test_download_input_stage_copies_local_file_into_workspace(tmp_path: Path) -> None:
    ...
    context = JobContext(
        job_id="job-123",
        request={"job_id": "job-123", "source": str(source_file)},
...
def test_download_input_stage_copies_file_uri_into_workspace(tmp_path: Path) -> None:
    ...
    context = JobContext(
        job_id="job-456",
        request={"job_id": "job-456", "source": source_file.as_uri()},
        workspace_dir=Path(job.root),
...
def test_download_input_stage_downloads_http_input_into_workspace(
    ...
    context = JobContext(
        job_id="job-789",
        request={"job_id": "job-789", "source": "https://example.com/media/test.mp3"},
        workspace_dir=Path(job.root),
...
def test_download_input_stage_raises_when_input_uri_missing(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="context.request.source is required"):
        stage.run(context)
...
def test_download_input_stage_raises_when_workspace_is_missing_and_no_job_metadata(
    ...
    context = JobContext(
        ...
        request={"job_id": "job-no-workspace", "source": str(source_file)},
        workspace_dir=None,
...
def test_download_input_stage_raises_for_missing_local_source(tmp_path: Path) -> None:
    ...
    context = JobContext(
        ...
        request={"job_id": "job-missing-file", "source": str(missing_file)},
        workspace_dir=Path(job.root),
...
def test_download_input_stage_raises_for_unsupported_uri_scheme(tmp_path: Path) -> None:
    ...
    context = JobContext(
        ...
        request={"job_id": "job-unsupported", "source": "s3://bucket/file.mp4"},
        workspace_dir=Path(job.root),
